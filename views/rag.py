...
if st.session_state.gemini_api_key:
    try:
        ...
        st.error(f"저장된 API Key 오류: {e}")

# -----------------------------------------------------------------------------
# 2. UI: API Key 입력 (메인 화면 상단)
# -----------------------------------------------------------------------------
...
with st.expander("🔑 Google Gemini API Key 설정", expanded=is_expanded):
    ...
    input_key = st.text_input(
        ...
        placeholder="sk-...",
        help="입력된 키는 브라우저를 닫을 때까지 유지됩니다."
    )
    ...
    if st.session_state.gemini_api_key:
        ...
        st.warning("API Key가 없습니다. RAG 분석 기능을 사용할 수 없습니다.")
    ...
    for idx, row in df_top5.iterrows():
        ...
        with st.container(border=True):
            ...
            with col_rag:
                ...
                if st.session_state.gemini_api_key and df_docs is not None and doc_embeddings is not None:
